...
fn hashmap_works_across_representations() {
    ...
    let key = OrderedNumber::new(Number::from(1) / Number::from(2));
    map.insert(key.clone(), "half");

    // Should find it regardless of how we construct 0.5
    let lookup = OrderedNumber::new(Number::from(1) / Number::from(2));
...
fn bigdecimal_division_flags_truncation() {
    ...
    let big =
        Number::try_from_i128_with_scale(1_000_000_000_000_000_000_000_000_000_000i128, 0).unwrap();
    let three = Number::from(3);
    let result = big / three;

    // Should be flagged as approximation since result is non-terminating
    assert!(
        result.is_rational_approximation() || result.representation() == "Rational",
